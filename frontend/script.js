...
function setupLoginScreen() {
    ...
    // เข้าสู่ระบบด้วย Google (ท่าไม้ตาย: Popup + Overlay)
    googleLoginBtn.onclick = async () => {
        try {
            // แสดงสถานะ "กำลังเตรียมการ..."
            ...
            // ถ้าสำเร็จ Firebase จะจัดการต่อใน onAuthStateChanged เอง
        } catch (error) {
            console.error('Google Sign In Error:', error);
            if (error.code === 'auth/popup-closed-by-user') {
                // ผู้ใช้ปิดหน้าต่างเอง ไม่ต้องขึ้น Alert ใหญ่
            } else {
                alert('ล็อกอินผิดพลาด: ' + error.message);
            }
        } finally {
            googleLoginBtn.disabled = false;
            googleLoginBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="18" height="18" style="margin-right:8px;">
